...
| POST | `/library/queue` | Add or play a library item — body `{ source_id, item_id, item_type, action, duration?, start_index?, start_item_id? }` (`duration` in seconds, 0–86400: for UPnP tracks, whose length MPD cannot know before decoding the stream — it feeds the queue display). `start_index` (0 = the first track) and `start_item_id` (that track's id, as the listing gave it) queue an album or a playlist **from one of its tracks** — the tracks before it are left out. The core reads the list again: a track that moved in between starts where it now is, one that left the list is **400**. HIGHRESAUDIO and Qobuz albums and playlists only — **400** with either field anywhere else |
...
| GET | `/library/playlists?source_id=` | The account's own playlists on a streaming source — the ones an item can be added to (HIGHRESAUDIO and Qobuz; on Qobuz, only the playlists the account owns, not those it follows) → `LibraryAlbum[]`, ids `mine:<n>`, description in `artist` (HIGHRESAUDIO) or the owner's name (Qobuz). A playlist just created, renamed or deleted is listed as it now is at once, even before the service's own list catches up. **400** for a source whose playlists cannot be edited |
| POST | `/library/playlists` | Create a playlist in that account — body `{ source_id, title (1–100 chars), description? (≤ 500) }` → `{ id, title }`. On Qobuz it is created **private**. **Do not retry it automatically**: an answer lost on the way back would create a second playlist |
| POST | `/library/playlists/add` | Add a track or a whole album to one of those playlists — body `{ source_id, playlist_id, item_id, item_type: "track"\|"album" }` → `{ added, already }`: tracks written, and tracks the playlist already held — nothing is ever added twice. `item_id` is a track's id as the library lists it (the player's `content_item_id` for the track playing now) or an album id. **400** refused, with the reason: a playlist that is not the account's own or no longer exists, a purchased (`vault:`) item, a write the service refused (a full Qobuz playlist, in Qobuz's own words). **503** / **504** the service failed or was too slow |
| POST | `/library/playlists/remove` | Take a track out of one of those playlists — body `{ source_id, playlist_id, item_id }` → `{ removed: 1\|0 }` (0: the playlist no longer held it). Every copy of the track goes. **400** / **503** / **504** as for `add`. Not retried automatically either |
| PUT | `/library/playlists` | Rename one of those playlists — body `{ source_id, playlist_id, title (1–100 chars), description? (≤ 500) }` → `{ id, title }`. An empty `description` clears it; **left out, the current one is kept**. **400** / **503** / **504** as for `add` |
| DELETE | `/library/playlists?source_id=&playlist_id=` | Delete one of those playlists → `{ deleted: true }`; it leaves `GET /library/playlists` at once. **Do not retry it automatically**: a deletion that went through answers **400** the second time. **400** / **503** / **504** as for `add` |
| GET | `/library/playlist-tracks?source_id=&playlist_id=` | The tracks of a playlist — the account's own or the service's (HIGHRESAUDIO and Qobuz) → `LibraryTrack[]`, every one of them however long the playlist. Numbered as listed: `track_number - 1` and `id` are the `start_index` and `start_item_id` that play the playlist from that track. **400** for a source whose playlists cannot be opened, and for a playlist that does not exist or no longer does (HIGHRESAUDIO: an account playlist). Open to guests, like the rest of the browse |
...
| GET | `/library/qobuz-playlists?type=editorial\|mine` | One of the two Qobuz playlist trees, as albums. Both trees share one id space, so an id goes to `qobuz-playlist-tracks` on its own. In `mine`, a playlist the account owns comes back as `mine:<id>` — the id `/library/playlists*` edit it by — and one it only follows keeps its bare id; every route that reads a playlist takes either form. `type` defaults to `editorial`; any other value is **422** |
| GET | `/library/qobuz-playlist-tracks?playlist_id=` | Tracks of a Qobuz playlist, of either tree — `playlist_id` bare or `mine:<id>`. Every track, however long the playlist; each keeps its number in its album (`/library/playlist-tracks` numbers them by their place). **400** for a playlist that does not exist or no longer does |
...
| GET | `/library/highresaudio-playlist-tracks?playlist_id=…&type=editorial\|mine` | Tracks of an HRA playlist — the same read as `GET /library/playlist-tracks`, which works for every source that has one. **400** for an account playlist that does not exist or no longer does |
...
| POST | `/qobuz/connection` | Start OAuth2 flow → `{ oauth_url }`. The core first downloads Qobuz's web player (9.2 MB) for its app credentials, so the answer may take a minute on a slow line: open the sign-in window on the click and point it at `oauth_url` once it arrives — a window opened after the answer is blocked by the browser past a few seconds. **502** with the reason in `detail`: Qobuz too slow (no byte for 30 s, or over 2 min in all), the connection cut, an answer over 64 MB, `play.qobuz.com` unreachable, or its format changed |
...
`POST /sysinfo/actions/restart-backend` answers **409** while the core is installing software, and `POST /sysinfo/actions/reboot` while anything is being installed on the box — by the core, from the Terminal, over SSH. Nothing is restarted; `detail` names what is being installed and is meant to be shown as is.

`POST /sysinfo/actions/update` body: `{ password, version?, token? }` → `{ status: "updating", from, to }`. Admin + **password** gated. Launches a **detached** updater (transient systemd unit) that reinstalls the core binary (to `version`, or latest when omitted), health-checks it, and **rolls back** on failure. `token` is an optional GitHub PAT for the private releases repo (Early Access); when omitted it falls back to the core's configured `RELEASE_DOWNLOAD_TOKEN`. Health-check requires the new binary to report the **target version** (not just answer `200`). Returns **409** if an update is already in progress (a crashed/stale in-progress state older than 15 min is ignored, so a dead updater can't wedge this), and while anything is being installed on the box — `detail` then names it, as for a refused reboot. Follow progress via `GET /sysinfo/update-status`.
...
| GET | `/packages/{package_id}/versions` | The major lines this package offers, when it lets you choose |
| GET | `/packages/{package_id}/notices` | What the package asks to be read before installing — the vendor's licence |
| POST | `/packages/{package_id}/install` | Install — `?version=` to pick a line, `?accept_notices=true` once its terms are accepted, body `{ web_password }` for a package with a web interface |
| POST | `/packages/{package_id}/web_password` | Set the web interface password of an installed package that has none — body `{ web_password }` |
| POST | `/packages/{package_id}/uninstall` | Uninstall — `?purge=true` to delete its settings too |
...
`GET /packages/` and `GET /packages/{package_id}` also return **`availability`** and **`availability_reason`**. `availability` is one of `available`, `unsupported` (the source genuinely publishes nothing for this box — permanent), `unknown` (the source could not be reached when the config was resolved — worth retrying) or `blocked` (another installed package forbids it, e.g. Roon Server next to Roon Bridge). `availability_reason` is a short sentence for display, `null` when nothing is wrong. `is_supported` keeps its meaning — can this box install it — and is now derived from the same facts; the two extra fields exist because a greyed-out button needs to say **why**, and because "no build for your machine" and "we could not ask" are not the same answer. A **vendor verdict outranks a local one**: a package with no build for this architecture reports `unsupported` even when a conflicting package is also installed, since removing the other one would not make it installable. Package ids are `airplay`, `mpd`, `naa`, `hqplayerd`, `upmpdcli`, `roon` and `roonserver`. `naa` is the Signalyst Network Audio Adaptor (`networkaudiod`), the local daemon — **it was called `hqplayer` until 0.9.59**, which collided with `/hqplayer/*`, the remote player. The two are different machines. The `is_test_package` flag stays in the response model, but no shipped package sets it: the no-op `test-dummy` entry that exercised the install workflow is no longer declared in the registry, so it is no longer resolved into a box's config. A **`required`** flag marks a package the box cannot work without: `POST /packages/{id}/uninstall` refuses it, and the interface offers no button for it. Only `mpd` sets it today — the local library, the queue, internet radio and all three streaming services play through MPD, so a box that lost it would keep an interface and lose the product. Stopping the service is still allowed, and is the way to run a box purely as a Roon endpoint or an HQPlayer NAA.

**`trial_notice`** is one sentence, in the vendor's own terms, naming what they limit until a licence key is entered — `null` for a package with no such limit. It is declared by the package, so a client shows it without knowing which package it is: before the install, and on the card afterwards. It exists because the core can **read** such a limit nowhere: HQPlayer Embedded without a licence stops taking commands 30 minutes after **the service starts** — not after the first note — while it keeps running and holds the DAC, and it writes nothing about that in any log, so `/hqplayer/status` reports it unavailable with no reason to give. Measured twice (2026-09-22), the second run playing nothing at all: last command accepted at 31.85 min of service uptime, refused at 32.19 min. Inferring the state from its signature is possible and tracked, not done.

Three more fields describe what happens around an install: **`web_credentials`** — `{ username, port, already_set }` for a package whose web interface needs a password (HQPlayer Embedded), `null` for the others; **`keeps_settings_on_uninstall`** — a plain uninstall keeps its settings, so deleting them too (`?purge=true`) is a choice to offer (true for the apt packages, false for a vendor script whose uninstall already deletes them); **`restarts_after_install`** — the core itself starts or restarts the package's service once it is installed or updated, so a client must not ask the operator to restart it afterwards, unless the action's result says `restart_needed`.

`GET /packages/{package_id}/versions` → `{ package_id, offers_choice, versions: [{ major, version, missing }], unavailable: [{ major, version, missing }] }`, newest line first and only the newest build of each. `offers_choice` mirrors **`offers_version_choice`** on the package itself, and is false for everything but HQPlayer Embedded, where the operator picks the line their licence covers. Each line's dependencies are checked with apt: a line this machine **cannot install** goes to `unavailable`, with `missing` naming what nothing provides — HQPlayer Embedded 5.17.2-48 needs `libgmpris` on Debian 13. `versions` holds the lines that can install, and also any whose dependencies could not be checked: "could not check" is not "cannot install", and the install checks again. A package that offers no choice answers with empty lists — asking is not an error. An empty `versions` **while `offers_choice` is true** means the vendor could not be reached. **403 for a guest.**

`GET /packages/{package_id}/notices?version=<v>` → `{ package_id, version, readable, notices: [{ title, body }] }`. The debconf notes the package would display while installing — screens Audiogravity never shows, because it installs with the noninteractive frontend. **Both Signalyst packages put their end-user licence agreement in one**, so it has to be shown before the install rather than skipped. Read from the package file itself, so it is always the vendor's current text; `version` reads the notes of the exact build about to be installed — **400** when that package offers no choice, or the vendor does not publish that version. **`readable: false`** means the package could not be read (vendor out of reach, a proxy dropping the Range header): `notices` is then empty for that reason and must **not** be taken for "nothing to accept". **403 for a guest.**

`POST /packages/{package_id}/install?version=<v>&accept_notices=true` installs that exact version. `version` is **refused** (`success: false`) for a package whose `offers_version_choice` is false, rather than ignored, and refused again if the vendor no longer publishes it — the version is matched against the listing at install time, not against the list shown earlier. Omit it to get what the package's own rule picks. **`accept_notices`** is required whenever the package shows terms (see `/notices`), and when they could not be read: without it the install is refused before anything happens. The core holds this, not the interface — the route answers anyone who calls it. Before downloading, the core also checks the package's dependencies with apt and the room it needs (root's free space, since dpkg runs as root, plus the download when it lands on the same disk); a package that fails either is refused with the reason in `message` and nothing changes on the box. The install is reported successful only once dpkg confirms the package is installed.

A package with **`web_credentials`** (HQPlayer Embedded) needs the password of its own web interface, sent in the **JSON body** as `{ "web_password": "…" }` — never in the query string, which ends up in access logs. It is required while `web_credentials.already_set` is false, and **refused** when it is true: existing credentials are never overwritten. A password for a package that has no `web_credentials` is refused too. Rules: 8 to 64 printable ASCII characters, no space, no leading `-`. The core sets it with the vendor's own tool right after the install, then restarts the service once so it reads it.

**Action results** — `install`, `update`, `uninstall`, `update_all` and `web_password` answer `PackageActionResult` `{ success, package_id, action, message, output, warning, restart_needed }`. **`success: true, warning: true`** means the package **is** installed or updated but a step after it was not done — its web password, its log space, its restart — or apt removed other packages on the way (named in `message`); `message` says what is left to do. Show it as a warning: "failed" would deny the part that was done. **`restart_needed: true`** means the package's service still runs on its old settings because the restart the core owed it failed — the one case where a client should offer to restart a package whose `restarts_after_install` is true. `install`, `update` and `uninstall` are **refused** (`success: false`, reason in `message`, nothing changed) while the core is stopping or updating itself and, for a package installed through apt, while another installation is running on the box or an earlier one left the package manager interrupted — `message` then gives the command that repairs it.

`POST /packages/{package_id}/web_password` body `{ web_password }` sets the password on an **installed** package whose `web_credentials.already_set` is false — for when the install could not set it, or the package was installed another way — then restarts its service so it uses it. Same rules as at install. **Refused** (`success: false`, reason in `message`, nothing changed) for a package without `web_credentials`, one not installed, one whose credentials already exist (they are changed from the package's own web interface), or an unusable password. The password never appears in `message`, `output` or the operation log. **403 for a guest.**
