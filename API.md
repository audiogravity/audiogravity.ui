...
- `audio_tuning` — per audio unit, what systemd is **configured** to apply against what the process **actually** carries: `configured` `{ nice, cpu_affinity, cpu_sched, cpu_sched_priority, io_class, io_priority, io_accounting, ip_accounting, drop_ins }` vs `live` `{ nice, cpu_sched, cpu_sched_priority, cpu_affinity, io }`, plus `restarts` (Restart=always masks crash loops), `started_at`, `memory_bytes`, `cpu_used_seconds`. The difference between the two halves is a drop-in that did not apply.
